#!/usr/bin/env bash
#SBATCH --job-name=smrnaseq_usrf_keep8
...
RUN_TAG="${RUN_TAG:-smrnaseq_keep8_$(date +%Y%m%d_%H%M%S)}"
...
SHORT_FILTER_DIR="${RUN_ROOT}/03.shortRNA_filtering"
FASTP_RESCUE_DIR="${SHORT_FILTER_DIR}/00.fastp_keep8nt"
FASTP_QC_DIR="${SHORT_FILTER_DIR}/qc"
...
CONDA_FASTP="${CONDA_ENV_PREFIX}/bin/fastp"
...
if [[ -x "${CONDA_FASTP}" ]]; then
  FASTP_BIN="${CONDA_FASTP}"
else
  FASTP_BIN="$(command -v fastp || true)"
fi

[[ -n "${FASTP_BIN}" ]] || { echo "ERROR: fastp not found"; exit 1; }
...
echo "git:      ${CONDA_GIT}"
echo "fastp:    ${FASTP_BIN}"

"${CONDA_NEXTFLOW}" -version
"${CONDA_JAVA}" -version
"${CONDA_PYTHON}" --version
"${CONDA_MAMBA}" --version
"${FASTP_BIN}" --version || true
...
export RAW_SHEET SE_SHEET PAIR_AUDIT
...
#############################################
# NEW: custom fastp rescue branch
#############################################
action_log "running custom fastp rescue branch: save passing reads with MINLEN=${MINLEN}"

export SE_SHEET FASTP_RESCUE_DIR FASTP_BIN MINLEN MAXLEN
...
se_sheet = Path(os.environ["SE_SHEET"])
outdir = Path(os.environ["FASTP_RESCUE_DIR"])
fastp = os.environ["FASTP_BIN"]
minlen = os.environ["MINLEN"]
maxlen = os.environ["MAXLEN"]
threads = os.environ.get("THREADS", "8")

df = pd.read_csv(se_sheet)
...
for _, r in df.iterrows():
    sample = clean_sample_name(r["sample"])
    r1 = str(r["fastq_1"])

    out_fq = outdir / f"{sample}.keep8nt.fastq.gz"
    html = outdir / f"{sample}.fastp.keep8nt.html"
    js = outdir / f"{sample}.fastp.keep8nt.json"
    log = outdir / f"{sample}.fastp.keep8nt.log"

    if out_fq.exists() and out_fq.stat().st_size > 0:
        print(f"exists: {out_fq}")
        continue
...
print("Custom fastp rescue complete")
PY

#############################################
# NEW: fastp rescue length QC
#############################################
action_log "building keep8nt length QC table"
...
indir = Path(os.environ["FASTP_RESCUE_DIR"])
qcdir = Path(os.environ["FASTP_QC_DIR"])
qcdir.mkdir(parents=True, exist_ok=True)

rows = []
for fq in sorted(indir.glob("*.keep8nt.fastq.gz")):
    sample = fq.name.replace(".keep8nt.fastq.gz", "")
    c = Counter()
    ...
    for length, count in sorted(c.items()):
        rows.append({"sample": sample, "length": length, "read_count": count})

df = pd.DataFrame(rows)
out = qcdir / "fastp_keep8nt_length_counts.tsv"
df.to_csv(out, sep="\t", index=False)

summary = df.groupby("length", as_index=False)["read_count"].sum().sort_values("length")
summary_out = qcdir / "fastp_keep8nt_length_counts.all_samples.tsv"
summary.to_csv(summary_out, sep="\t", index=False)

below_11 = int(summary.loc[summary["length"] < 11, "read_count"].sum()) if not summary.empty else 0
total = int(summary["read_count"].sum()) if not summary.empty else 0

print(f"Saved: {out}")
print(f"Saved: {summary_out}")
print(f"Total reads: {total:,}")
print(f"Reads <11 nt: {below_11:,}")
print(summary[(summary["length"] >= 8) & (summary["length"] <= 20)].to_string(index=False))
PY
...
def resolve_env_target(nf_file: Path, value: str):
    ...
    if "environment.yml" in raw or "environment.yaml" in raw:
        ...
        return {"kind": "yaml", "source": str(yml_path), "env_prefix": str(env_prefix), "conda_value": str(env_prefix)}
    else:
        env_prefix = env_root / spec_env_name(raw)
        return {"kind": "spec", "source": raw, "env_prefix": str(env_prefix), "conda_value": str(env_prefix)}
...
#############################################
# Run patched local nf-core pipeline
#############################################
action_log "launching patched local nf-core pipeline"
...
find "${FASTP_RESCUE_DIR}" -type f -name "*.keep8nt.fastq.gz" | while read -r f; do
    ln -sf "$f" "${RESID_DIR}/$(basename "$f")"
done
...
echo "Single-end samplesheet:     ${SE_SHEET}"
echo "Pair audit:                 ${PAIR_AUDIT}"
echo "Custom keep8 fastp reads:   ${FASTP_RESCUE_DIR}"
echo "Keep8 length QC:            ${FASTP_QC_DIR}/fastp_keep8nt_length_counts.all_samples.tsv"
